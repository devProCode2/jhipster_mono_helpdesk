...
type RestOf<T extends IPessoa | NewPessoa> = Omit<T, 'dataCadastro'> & {
...
export class PessoaService {
  ...
  protected convertDateFromClient<T extends IPessoa | NewPessoa | PartialUpdatePessoa>(pessoa: T): RestOf<T> {
    return {
      ...pessoa,
      dataCadastro: pessoa.dataCadastro?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPessoa: RestPessoa): IPessoa {
    return {
      ...restPessoa,
      dataCadastro: restPessoa.dataCadastro ? dayjs(restPessoa.dataCadastro) : undefined,
    };
  }
